...
import { KiwiResponseSchema, type KiwiResponse, type KiwiSizing } from './schemas.js';
import type { SizeChartSection, TableSection, TextSection } from './types.js';
 ...
 */
export function detectKiwiSignature(html: string): KiwiSignature | null {
  const shopMatch = html.match(/KiwiSizing\.shop\s*=\s*"([^"]+)"/);
  const productMatch = html.match(/\bproduct:\s*"(\d+)"/);
  if (!shopMatch || !productMatch) return null;
 ...
 */
export function buildKiwiChartUrl(sig: KiwiSignature): string {
...
export function parseKiwiResponse(
  raw: unknown,
  shopCountryCode: string | null = null,
): ParsedKiwiChart | null {
...
function sizingToSections(
  ...
  for (const entry of layout) {
    if (entry.type === 0 && typeof entry.value === 'string') {
      ...
      if (!table?.data || table.data.length < 2) continue;
      ...
      for (let i = 1; i < table.data.length; i++) {
        ...
        const row: Array<string | number> = sourceRow.map((cell, idx) => {
        ...
        if (row.length > rawHeaders.length) row.length = rawHeaders.length;
        rows.push(row);
      }
...
export function sanitizeKiwiHtml(html: string): string {
  let out = html
